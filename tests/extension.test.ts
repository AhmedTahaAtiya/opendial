...
test('extension artifact contains local app with minimal secure manifest', async () => {
  ...
  assert.ok(
    !names.some(
      (name) => name.endsWith('.map') || name.startsWith('.env') || name.includes('node_modules'),
    ),
  );
  ...
  const text = (
    await Promise.all(
      names
        .filter((name) => /\.(html|js|css|json|txt)$/.test(name))
        .map((name) => readFile(resolve(output, name), 'utf8')),
    )
  ).join('\n');
