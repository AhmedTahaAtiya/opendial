...
const STATIC_ASSETS = ['index.html', 'manifest.json', 'icon.svg', 'icons/icon-16.png', 'icons/icon-32.png', 'icons/icon-48.png', 'icons/icon-128.png'];
...
export async function generateExtensionZip(backupData: ExportBackupData): Promise<Blob> {
  ...
  const contents = await Promise.all(STATIC_ASSETS.map(function (path) { return addAsset(zip, path); }));
  ...
  await Promise.all(paths.map(function (path) { return addAsset(zip, path); }));
  zip.file('README.txt', 'OpenDial offline New Tab extension. Extract and load unpacked in a Chromium browser.\n');
